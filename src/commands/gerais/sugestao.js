    ...
    constructor() {
        return {
            ...
            args: [
                {
                    name: 'mensagem',
                    desc: 'A mensagem que deja enviar para a equipe.',
                    type: 'STRING',
                    required: true,
                    autocomplete: false
                }
            ...
            argsEn: [
                {
                    name: 'message',
                    desc: 'The message you want to send to the team.',
                    type: 'STRING',
                    required: true,
                    autocomplete: false
                }
            ...
            type: 1,
            helpPt: {
                title: '<:outrosAjuda:766790214110019586> ' + '/' + 'suporte',
                desc: `
            ...
            **${'/'}suporte <mensagem>**
            ...
            helpEn: {
                title: '<:outrosAjuda:766790214110019586> ' + '/' + 'suporte',
                desc: `This command is used to send a bug report or suggestion to my creator, it's also used to remove questions, but, be patient, there's not a team behind me is just one person to do everything. To use the command just follow the example:
            ...
            const embed = new client.Discord.EmbedBuilder()
                ...
                .setFooter({
                    text:
                        ...
                        time.now({ zone: 'America/Fortaleza' }).toFormat('dd/MM/y | HH:mm:ss ') +
                        '(GMT -3)'
                });

            client.log.embed(embed, true, 'sugestao').then(msg => {
                client.shard.broadcastEval((c, ctx) => {
                    const { msg } = JSON.parse(ctx);

                    const channel = c.channels.cache.get(process.env.LOG_CHANNEL);

                    if (channel) {
                        const message = channel.messages.cache.get(msg.id);

                        if (message) {
                            message.pin();
                        }
                    }
                }, { context: JSON.stringify({ msg: msg }) });
            });
