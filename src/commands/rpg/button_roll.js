    ...
    constructor() {
        return {
            ...
            args: [
                {
                    name: 'dados',
                    desc: 'Dados que devem ser criados botões, no máximo 25, separados por "|" e sem espaços',
                    type: 'STRING',
                    required: true,
                    autocomplete: false
                }
            ...
            argsEn: [
                {
                    name: 'dices',
                    desc: 'Dices that should be created buttons, maximum 25, separated by "|" and without spaces',
                    type: 'STRING',
                    required: true,
                    autocomplete: false
                }
            ...
            type: 1,
            helpPt: {
                title: '<:dadosAjuda:766790214030852137> ' + '/' + 'buttonroll',
                desc: `
            ...
            dados.forEach(dado => {
                ...
                for (var x in testSize) {
                    ...
                    if (Number(qdados) > 10000 || Number(tdados) > 100000000 || Number(bdados) > 100000000) {
                        return int.editReply({
                            content: client.tl({ local: int.lang + 'btR-tDadoMA', cmd: numberDice })
                        });
                    }

                    if (Number(qdados) <= 0 || Number(tdados) <= 0 || Number(bdados) < 0) {
                        return int.editReply({
                            content: client.tl({ local: int.lang + 'btR-tDadoMB', cmd: numberDice })
                        });
                    }
                ...
                try {
                    if (segments.length > 1) {
                        ...
                        if (!pass) {
                            return int.editReply({
                                content: client.tl({ local: int.lang + 'btR-dInv', cmd: numberDice })
                            });
                        }
                        ...
                        if (segments[0].search('d') == -1) {
                            if (Number(segments[0]) > 100000000) {
                                return int.editReply({
                                    content: client.tl({ local: int.lang + 'btR-dInv', cmd: segments[0] })
                                });
                            }
