...
export default {
    ownerOnly: false,
    commandNames: {
        'pt-br': 'ficha_apagar_atributo',
        'en-us': 'sheet_delete_attribute'
    },
    fullNames: {
        'pt-br': 'Apagar Atributo',
        'en-us': 'Delete Attribute'
    },
    descriptions: {
        'pt-br': 'Remove um atributo de uma seção da ficha.',
        'en-us': 'Remove an attribute from a sheet section.'
    },
    arguments: {
        'pt-br': [
            {
            ...
            {
                name: 'atributo',
                description: 'Atributo a ser removido.',
                type: 'STRING',
                required: true,
                autocomplete: true
            }
    ...
    run: async (int: Interaction, language: Available_Languages) => {
        ...
        const validated = await SheetServices.validateDeleteAttribute(sheet, section, attribute);
        ...
        const buttonConfirm = new ButtonBuilder()
            .setCustomId(`$a$confirm-delete-attribute|${tempId}`)
            .setLabel(localization(language, 'sheet|confirm-delete-button'))
            .setStyle(ButtonStyle.Danger);

        const buttonCancel = new ButtonBuilder()
            .setCustomId(`$a$cancel-delete-attribute|${tempId}`)
        ...
        const msg = (await int.reply({
            content: localization(language, 'sheet|confirm-delete-attribute-message', [
        ...
        actionHandler.registerAction(`$a$confirm-delete-attribute|${tempId}`, {
            action: async (comp: Interaction) => {
                ...
                try {
                    ...
                    await rest.patch(Routes.webhookMessage(int.application_id, comp.token, msg.id), {
                        body: {
                            content: localization(language, 'sheet|attribute-deleted'),
                            components: []
                        },
                        headers: { 'Content-Type': 'application/json' }
                    });
                } catch (err) {
                    logger.logText('ERROR', `Error deleting attribute: ${err}`);
                }
            },
            singleUse: true,
        ...
        actionHandler.registerAction(`$a$cancel-delete-attribute|${tempId}`, {
            action: (comp: Interaction) => {
                ...
                rest.patch(Routes.webhookMessage(int.application_id, comp.token, msg.id), {
                    ...
                    logger.logText('ERROR', `Error updating cancel delete attribute message: ${err}`);
                });
            },
            singleUse: true,
            respondOnlyToUserId: int.user.id
