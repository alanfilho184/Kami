...
function default_id_validation(id: string, type: string) {
    if (!id) {
        ...
        throw new ValidationError(
            ...
            `The ${type} must have between 18 and 20 digits of length`
        );
    } else {
        return id;
    }
...
class Sheet_Name {
    ...
    constructor(sheet_name: string) {
        if (sheet_name.length > 32) {
            throw new ValidationError(
                `"${sheet_name}" is not a valid sheet_name`,
                'Exceeded the maximum of 32 characters'
            );
...
class Macro_Name {
    ...
    constructor(macro_name: string) {
        if (macro_name.length > 32) {
            throw new ValidationError(
                `"${macro_name}" is not a valid macro_name`,
                'Exceeded the maximum of 32 characters'
            );
